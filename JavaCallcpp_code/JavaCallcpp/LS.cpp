...
void CLS::addPoint(double y)
{
	m_VertexY.push_back(y);
	m ++;
}
...
bool CLS::Solve2()
{
	...
	for (i = 1; i < n; i ++)
	{
		for (j = 0; j < m; j ++)
		{
			A->setData(j,i,A->getData(j,i-1) * (j + 1));//pVertex[j][0]
		}
	}
	CMatrix *B = A->getRev();
	CMatrix *b = new CMatrix(m,1);
	for(i = 0; i < m; i ++)
		b->setData(i,0,m_VertexY[i]);//pVertex[i][1]
