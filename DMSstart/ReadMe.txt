...
应用程序向导已为您创建了此 DMSstart 项目，您可以在此基础之上
编写可执行程序 (EXE)。
...
DMSstart.idl
    ...
    和 coclass 的 IDL 定义。
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明                 (DMSstart.h)
        GUID 定义                                (DMSstart_i.c)
        一个类型库                               (DMSstart.tlb)
        封送代码                                 (DMSstart_p.c 和 dlldata.c)

DMSstart.h
    此文件包含在 DMSstart.idl 中定义的项的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 DMSstart.pch 的预编译头 (PCH) 文件
...
DMSstartps.vcproj
	...
	dlldata.c、DMSstart_i.c 和 DMSstart_p.c，它们。
