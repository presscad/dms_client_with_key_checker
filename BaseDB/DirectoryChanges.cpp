...
//
//
void CDirectoryChangeHandler::On_FileAdded(const ULONGLONG id,const CTime time,const CString & strFileName)
{ 
	TRACE(_T("The following file was added: %s\n"), strFileName);
}

void CDirectoryChangeHandler::On_FileRemoved(const ULONGLONG id,const CTime time,const CString & strFileName)
{
	TRACE(_T("The following file was removed: %s\n"), strFileName);
}

void CDirectoryChangeHandler::On_FileModified(const ULONGLONG id,const CTime time,const CString & strFileName)
{
	TRACE(_T("The following file was modified: %s\n"), strFileName);
}

void CDirectoryChangeHandler::On_FileNameChanged(const ULONGLONG id,const CTime time,const CString & strOldFileName, const CString & strNewFileName)
{
									   ...
									   )
{
	...
	CDirWatchInfo * pDirInfo = new CDirWatchInfo( hDir, strDirToWatch, pBaseDirMonitor, dwChangesToWatchFor, bWatchSubDirs, m_bAppHasGUI, szIncludeFilter, szExcludeFilter, m_dwFilterFlags);
	if( !pDirInfo )
	{
	...
	//slfAdd
	//CBaseSupport::InitFileDirID(pDirInfo->m_StartID,strDirToWatch);
...
{ 
	...
	m_pBaseDirMonitor = pBaseDirMonitor;
...
********************************************/
{
	...
	//CTime ctime = pThis->m_startTime;

	//CTime startTime(ctime.GetYear(),ctime.GetMonth(),ctime.GetDay(),0,0,0);//(,,ctime.GetDay(),0,0,0);

	//if(s_MapFileMaxTime[])

	//ULONGLONG totalId = month * 100 + ctime.GetDay();
	//totalId *= 1000;
				...
				case CDirWatchInfo::RUNNING_STATE_NORMAL:
					{
						...
						ULONGLONG id = time.GetTime() - pdi->m_pBaseDirMonitor->GetStartID();//time.GetTime() - pThis->m_startTime.GetTime();
						//id -= 1529000000;
						id *= 1000;
						...
						//ULONGLONG id = totalId + cnt * 10;
						pThis->ProcessChangeNotifications(notify_info, pdi, dwReadBuffer_Offset,time,id);
...
DWORD WINAPI ThreadProcFileMonitor(LPVOID lpParam)  
{ 
...
DWORD WINAPI ThreadProcFileMonitor2(LPVOID lpParam)  
{ 
	THREAD_PARAM*  pThreadPamam = (THREAD_PARAM*)lpParam;
	CFileBaseMonitor* fileMonitor = (CFileBaseMonitor*)pThreadPamam->mpParam;
	fileMonitor->FileChanged2(pThreadPamam/*->cType,pThreadPamam->id,pThreadPamam->batchID,pThreadPamam->mtime,pThreadPamam->mStrRootPath,pThreadPamam->mFileName,pThreadPamam->mNewFileName*/);
	//TRACE("%d\t%s\t%s\n",pThreadPamam->cType,pThreadPamam->mFileName,pThreadPamam->mNewFileName);
	delete pThreadPamam;
	return 0;
}
...
DWORD WINAPI ThreadProcFileMonitor3(LPVOID lpParam)  
{ 
	THREAD_PARAM*  pThreadPamam = (THREAD_PARAM*)lpParam;
	CFileBaseMonitor* fileMonitor = (CFileBaseMonitor*)pThreadPamam->mpParam;
	fileMonitor->FileChanged3(pThreadPamam/*->cType,pThreadPamam->id,pThreadPamam->batchID,pThreadPamam->mtime,pThreadPamam->mStrRootPath,pThreadPamam->mFileName,pThreadPamam->mNewFileName*/);
	//TRACE("%d\t%s\t%s\n",pThreadPamam->cType,pThreadPamam->mFileName,pThreadPamam->mNewFileName);
	delete pThreadPamam;
	return 0;
}

DWORD WINAPI ThreadProcFileMonitor4(LPVOID lpParam)  
{ 
	THREAD_PARAM*  pThreadPamam = (THREAD_PARAM*)lpParam;
	CFileBaseMonitor* fileMonitor = (CFileBaseMonitor*)pThreadPamam->mpParam;
	fileMonitor->FileChanged4(pThreadPamam/*->cType,pThreadPamam->id,pThreadPamam->batchID,pThreadPamam->mtime,pThreadPamam->mStrRootPath,pThreadPamam->mFileName,pThreadPamam->mNewFileName*/);
	//TRACE("%d\t%s\t%s\n",pThreadPamam->cType,pThreadPamam->mFileName,pThreadPamam->mNewFileName);
	delete pThreadPamam;
	return 0;
}
														 ...
														 ,CTime time ,ULONGLONG id)
{
	...
	CString strFileName = notify_info.GetFileNameWithPath(pdi->m_strDirName);
	CFileStatus rStatus;
	rStatus.m_size = 0;
	...
	//s_MapLastFileTime[pdi->m_strDirName] = time;slfAdd
	//CBaseSupport::SetMonitorLastFileTime(pdi->m_strDirName,time);

	//pdi->m_pBaseDirMonitor->SetLastTime(time);
	TRACE("--------------------------------");
	ULONGLONG batchID = pdi->m_pBaseDirMonitor->GetStartID();
	//int index = 0;
    do
	{
		...
		case FILE_ACTION_ADDED:		// a file was added!
			...
			{
				CString strRootPath = pdi->m_strDirName;
				CString fileName = notify_info.GetFileName();
				//CTime time = CTime::GetCurrentTime();
				THREAD_PARAM* pParam = new THREAD_PARAM(1,id ++,batchID,CBaseSupport::sGetFileBaseMonitor(),time,strRootPath,fileName,"",pdi->m_pBaseDirMonitor);
				//id ++;
				HANDLE hThread = CreateThread(NULL, 0, ThreadProcFileMonitor, pParam, 0, NULL);
				if(INVALID_HANDLE_VALUE == hThread){
					delete pParam;
				}
				TRACE("Added£º%I64d\t%s\t%s\n",id,strRootPath,fileName);
			}
		...
		case FILE_ACTION_REMOVED:	//a file was removed
		
			{
				CString strRootPath = pdi->m_strDirName;
				CString fileName = notify_info.GetFileName();
				THREAD_PARAM* pParam = new THREAD_PARAM(2,id++,batchID,CBaseSupport::sGetFileBaseMonitor(),time,strRootPath,fileName,"",pdi->m_pBaseDirMonitor);
				HANDLE hThread = CreateThread(NULL, 0, ThreadProcFileMonitor2, pParam, 0, NULL);
				if(INVALID_HANDLE_VALUE == hThread){
					delete pParam;
				}
				TRACE("removed£º%I64d\t%s\t%s\n",id,strRootPath,fileName);
			}
			break;
		case FILE_ACTION_MODIFIED:
			{
				CString strRootPath = pdi->m_strDirName;
				CString fileName = notify_info.GetFileName();
				THREAD_PARAM* pParam = new THREAD_PARAM(3,id ++,batchID,CBaseSupport::sGetFileBaseMonitor(),time,strRootPath,fileName,"",pdi->m_pBaseDirMonitor);
				HANDLE hThread = CreateThread(NULL, 0, ThreadProcFileMonitor3, pParam, 0, NULL);
				if(INVALID_HANDLE_VALUE == hThread){
					delete pParam;
				}
				TRACE("modified£º%I64d\t%s\t%s\n",id,strRootPath,fileName);
			}
			break;
		case FILE_ACTION_RENAMED_OLD_NAME:
			{//a file name has changed, and this is the OLD name
			 ...
			 //the action set to FILE_ACTION_RENAMED_NEW_NAME (contains the new name of the file
				...
				CString strOldFileName = notify_info.GetFileName();
				...
				{//there is another PFILE_NOTIFY_INFORMATION record following the one we're working on now...
				 //it will be the record for the FILE_ACTION_RENAMED_NEW_NAME record
					...
					CString strNewFileName = notify_info.GetFileName( );

					{
						CString strRootPath = pdi->m_strDirName;
						CString fileName = notify_info.GetFileName();
						THREAD_PARAM* pParam = new THREAD_PARAM(4,id++,batchID,CBaseSupport::sGetFileBaseMonitor(),time,strRootPath,strOldFileName,strNewFileName,pdi->m_pBaseDirMonitor);
						HANDLE hThread = CreateThread(NULL, 0, ThreadProcFileMonitor4, pParam, 0, NULL);
						if(INVALID_HANDLE_VALUE == hThread){
							delete pParam;
						}
						TRACE("rem£º%I64d\t%s\t%s\n",id,strRootPath,fileName);
					}
